...
CREATE TABLE SHOPLIST.article (
	...
	EAN13 varchar(13) NULL) ENGINE = INNODB;
...
CREATE TABLE SHOPLIST.list (
	...
	CREATE_DATE date NULL) ENGINE = INNODB;
...
CREATE TABLE SHOPLIST.SHOP_LIST_L_ARTICLE (
	...
	STATUS varchar(5) DEFAULT 'BUY' NOT NULL) ENGINE = INNODB;
...
CREATE TABLE SHOPLIST.shelf (
	...
	NAME varchar(100) NOT NULL,
	`POSITION` integer NOT NULL) ENGINE = INNODB;
ALTER TABLE SHOPLIST.shelf ADD CONSTRAINT SHELF_SHOP_SHELF_PK PRIMARY KEY (CODE);
CREATE TABLE SHOPLIST.user (
	LOGIN varchar(10) NOT NULL,
	NAME varchar(100) NOT NULL,
	`PASSWORD` varchar(100) NULL,
	PROFILE varchar(10) DEFAULT 'USER' NOT NULL) ENGINE = INNODB;
ALTER TABLE SHOPLIST.user ADD CONSTRAINT USER_SHOP_USER_PK PRIMARY KEY (LOGIN);

-- Foreign key constraints 
ALTER TABLE SHOPLIST.article ADD CONSTRAINT SHOP_ARTICLE_L_SHELF FOREIGN KEY (SHELF) REFERENCES SHOPLIST.shelf (CODE) ON DELETE RESTRICT ON UPDATE CASCADE;
ALTER TABLE SHOPLIST.list ADD CONSTRAINT SHOP_ARTICLE_L_USER FOREIGN KEY (`USER`) REFERENCES SHOPLIST.user (LOGIN) ON DELETE RESTRICT ON UPDATE CASCADE;
ALTER TABLE SHOPLIST.SHOP_LIST_L_ARTICLE ADD CONSTRAINT SHOP_LIST_L_ARTICLE_L_LIST FOREIGN KEY (LIST_ID) REFERENCES SHOPLIST.list (ID) ON DELETE CASCADE ON UPDATE CASCADE;
ALTER TABLE SHOPLIST.SHOP_LIST_L_ARTICLE ADD CONSTRAINT SHOP_LIST_L_ARTICLE_L_ARTICLE FOREIGN KEY (ARTICLE_ID) REFERENCES SHOPLIST.article (ID) ON DELETE RESTRICT ON UPDATE CASCADE;
