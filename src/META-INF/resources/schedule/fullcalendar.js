...
//			htmlEscape(event.title) +
			(opt('htmlTitle') ? event.title : htmlEscape(event.title)) +
			"</div>" +
			"</div>" +
...
//				"<span class='fc-event-title'>" + htmlEscape(event.title) + "</span>" +
				"<span class='fc-event-title'>" + (opt('htmlTitle') ? event.title : htmlEscape(event.title)) + "</span>" +
				"</div>";
			if (seg.isEnd && isEventResizable(event)) {
				html +=
...
(function($, undefined) {
	var fullCalendarInit = $.fn.fullCalendar;

	$.fn.fullCalendar = function(options) {
		var calendar = fullCalendarInit.apply(this, arguments);

		this.each(function(i, _element) {
			var element = $(_element);
			var id = element.attr('id');

			if (element.find('.fc-header-left').find('input').length == 0) {
				var input = $('<input type="text" size="10" id="' + id + '-input" style="width: auto;" />');
				input.change(function(event) {
					var str = this.value;

					try {
						// TODO bazint fullCalendar Gérer I18N.
						if (str.match(/^\d{2}\/[\d]{2}$/)) {
							element.fullCalendar('gotoDate', $.datepicker.parseDate('dd/mm', str));
						} else if (str.match(/^\d{2}\/\d{2}\/(\d{2}|\d{4})$/)) {
							element.fullCalendar('gotoDate', $.datepicker.parseDate('dd/mm/yy', str));
						} else {
							this.value = '';
						}
					} catch (e) {
						// Invalid date (ex: 29/02/13)
						this.value = '';
					}
				});
				...
				element.find('.fc-header-left').first().append($('<span class="fc-header-space" />')).append(input);
			}

			var tabs = element.parents('.ui-tabs');

			if (tabs.length > 0) {
				tabs.each(function() {
					$(this).tabs({
					    activate: function(event, ui) {
					        $('#' + id.replaceAll(':', '\\:')).fullCalendar('render');
					    }
					});
				});
			}
...
function filterEvents(filter, scheduleId) {
	var schedule = $('#mainForm\\:' + scheduleId);
	var events = schedule.fullCalendar('clientEvents');
	...
	if (filter != undefined && filter.length > 0) {
		...
		for (var i = 0; i < events.length; i++) {
			var event = events[i];
			var str = event.id.toLowerCase() + ' ' + event.title.toLowerCase();
			var filtered = false;

			for (var j = 0; j < filterLen && !filtered; j++) {
				if (str.indexOf(filters[j]) < 0) {
					filtered = true;
				}
			}
