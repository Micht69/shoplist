...
//            $title.text(this.$current.data("jwizard-title"));
        	...
        	for (var i = 0 ; i < this.$current.index(); i++) {
        		titleText += $(this.$steps.get(i)).data("jwizard-title") + " >> ";
        	}
        	titleText += this.$current.data("jwizard-title");
    ...
    _buildSteps: function () {
        ...
        if (this.options.initialStep) {
        	this.$current = this.options.initialStep.show();
        } else {
...
(function($, undefined) {
	...
	$.widget("db.jWizard", $.db.jWizard, {
		...
		step : function($step) {
			...
			var currentStep = $(wizard.$current);
			var wizardId = wizard.element.attr('id');
			var clientId = wizardId.substring(0, wizardId.length - 7);
			var data = {
					'javax.faces.partial.ajax': true,
					'javax.faces.partial.execute': clientId + " @component",
					'javax.faces.partial.render': clientId + " @component mainForm:messages",
					'javax.faces.source' : clientId,
					'javax.faces.ViewState': $('#javax\\.faces\\.ViewState').last().val()
			};
			data[clientId + "_nextStepRequest"] = $step.attr('id').replace("mainForm:", "");

			if ($step.index() < currentStep.index()) {
				data[clientId + "_backRequest"] = true;
			}
			...
			$.ajax({
				...
				success: function(data, status, xhr) {
					...
					if (redirect.length > 0) {
						window.location.assign(redirect.attr('url'));
						return false;
					}

					var divUpdate = $($(xmlDoc.find('update[id="' + clientId + '"]').first()).text());
					var msgUpdate = $($(xmlDoc.find('update[id="mainForm\\:messages"]').first()).text());
					var viewStateUpdate = $(xmlDoc.find('update[id="javax\\.faces\\.ViewState"]').first());

					if (msgUpdate.children().length > 0) {
						$('#mainForm\\:messages').html(msgUpdate.children());
					} else {
						$('#mainForm\\:messages').html("");
					}

					if (divUpdate.attr('id') != currentStep.attr('id')) {
						$step.html(divUpdate.children());
						wizard._leave(wizard.$current).then(function() {
							wizard._enter($step).then(proceed, goback);
						}, goback);
					} else {
						currentStep.html(divUpdate.children());
						wizard._enableButtons();
					}
					$('#javax\\.faces\\.ViewState').last().val(viewStateUpdate.text());
				}
			});
			return dfd.promise();
		}
