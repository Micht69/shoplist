...
public class TreeTableModel implements Serializable {
	 ...
	 * serial UID
	 */
	private static final long serialVersionUID = -9208883632850998100L;
	...
	public TreeTableModel(Results results) {
		Map<String, BreakNode> nodeMap = new HashMap<String, BreakNode>();

		root = new DefaultTreeNode("root", null);
		List<String> breakers = results.getCategoryBreak();
		...
		for (String b : breakers) {
			...
			breakColumnCaption += results.getTitles().get(b);
		}
	}
	...
	public List<Key> getSelectedPk() {
		...
		for (TreeNode n : selectedNodes) {
			if (n instanceof RowNode) {
				addOnceToList(((Result) n.getData()).getPk(), keys);
			}
